...
export default function Hero() {
  ...
  useGSAP(
    () => {
      ...
      gsap.set(q(".hero-eyebrow, .hero-title, .hero-subtitle, .hero-button"), {
        willChange: "transform,opacity",
      });

      const tl = gsap.timeline({ defaults: { ease: "power2.out" } });

      tl.from(q(".hero-eyebrow"), { y: 16, autoAlpha: 0, duration: 0.45 })
        .from(q(".hero-title"), { y: 36, autoAlpha: 0, duration: 0.8 }, "-=0.2")
        .from(
          q(".hero-subtitle"),
    ...
    >
      <Container className="relative">
        <div className="max-w-2xl">
          <p className="hero-eyebrow mb-4 text-[12px] font-medium uppercase tracking-[0.16em] text-neutral-500 sm:mb-5">
            <span className="sm:hidden">Nadzor • Izvedba</span>
            <span className="hidden sm:inline">
              Nadzor • Izvedba • Odgovornost
            </span>
          </p>
