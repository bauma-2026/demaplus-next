  ...
  return (
    <section className="border-t border-neutral-200 bg-neutral-50">
      <Container className="py-12 sm:py-14">
        ...
        {/* MOBILE / TABLET */}
        <div className="mt-6 divide-y divide-neutral-200 lg:hidden">
          {listItems.map((p) => (
            <Link
              key={p.slug}
              href={`/reference/${p.slug}`}
              className="group grid gap-4 py-5 sm:grid-cols-[180px_1fr] sm:items-center sm:gap-5 sm:py-6"
            >
              ...
              <div className="min-w-0">
                ...
                <h3 className="mt-1.5 text-base font-semibold tracking-tight text-neutral-900 transition-colors duration-300 group-hover:text-neutral-700 sm:text-lg">
                  {p.title}
                </h3>

                <p className="mt-1.5 text-sm text-neutral-600 transition-colors duration-300 group-hover:text-neutral-900">
        ...
        {/* DESKTOP */}
        <div className="mt-8 hidden lg:grid lg:grid-cols-3 lg:gap-x-6 lg:gap-y-7">
          {listItems.map((p, idx) => (
            <Link
              key={p.slug}
              href={`/reference/${p.slug}`}
